    ...
    try:
        ...
        if parsed_shift_data_list:
            ...
            for shift_info in parsed_shift_data_list:
                ...
                # ★★★★★ ここからが重複チェックのロジックです ★★★★★
                # 1. Googleカレンダーに同じようなイベントがないか確認
                existing_event = await calendar_service.find_existing_event(db_client, user_id, shift_info)

                if existing_event:
                    # 2a. 重複が見つかった場合
                    print(f"DUPLICATE_FOUND: [{user_id}] Shift on {date_str} already exists in Calendar. Skipping.")
                    parse_results_for_reply.append(f"- {date_str}: {name_s}{start_t}～{end_t} {role_s}{memo_s} -> 既にカレンダーに登録済みです".strip())
                
                else:
                    # 2b. 重複がない場合のみ、カレンダー登録に進む
                    print(f"NO_DUPLICATE: [{user_id}] Proceeding to register shift on {date_str}.")
...
async def line_webhook_callback(request: Request, background_tasks: BackgroundTasks):
    ...
    try:
        events = line_webhook_handler.parser.parse(body, signature)
...
def handle_text_message(event: MessageEvent):
    ...
    try:
        ...
        if "連携" in received_text:
            reply_msg = "Googleカレンダー連携はLIFFアプリから確認・設定できます。"
        
        line_bot_api.reply_message(ReplyMessageRequest(
            reply_token=reply_token,
            messages=[MessagingTextMessage(text=reply_msg)]
        ))
...
async def handle_follow_event(db_client: Optional[firestore.Client], event: FollowEvent):
    ...
    try:
        ...
        print(f"INFO_WEBHOOK: Retrieved profile for {line_user_id}, display_name: {display_name}")
    except Exception as e:
        print(f"WARNING_WEBHOOK: Could not get user profile for {line_user_id}. Error: {e}")

    success = await firestore_service.create_initial_user_document_on_follow(db_client, line_user_id, display_name)
    
    if success:
        message_text = f"{display_name or 'あなた'}さん、友だち追加ありがとうございます！シフト管理ボットです。"
        liff_url = getattr(settings, 'LIFF_URL_GOOGLE_AUTH', None)
        if liff_url:
            message_text += (
        ...
        try:
            line_bot_api.reply_message(ReplyMessageRequest(
                reply_token=reply_token,
                messages=[MessagingTextMessage(text=message_text)]
            ))
